...
def test_PixelQuality():
	...
	# When quality is all zeros, filter should give all True:
	assert all(PixelQualityFlags.filter(quality))

	# Assign a random flag to one of the qualities and see if it filters:
	quality[3] = PixelQualityFlags.ManualExclude | PixelQualityFlags.BackgroundShenanigans
	indx = TESSQualityFlags.filter(quality, PixelQualityFlags.ManualExclude)
	assert not indx[3]
	assert sum(indx) == len(quality) - 1

	# Test binary representation:
	rep = PixelQualityFlags.binary_repr(quality)
	print(rep)
	assert len(rep) == len(quality)
	assert len(rep[3]) == 32
	assert rep[3] == '00000000000000000000000000000110'
	assert rep[0] == '00000000000000000000000000000000'

	rep = PixelQualityFlags.binary_repr(2051)
	print(rep)
	assert rep == '00000000000000000000100000000011'

	# Test decoding to strings:
	dc = PixelQualityFlags.decode(quality[3])
	print(dc)
	assert len(dc) == 2
	assert 'Manual exclude' in dc

	# A default bitmask should be set:
	dmask = PixelQualityFlags.DEFAULT_BITMASK
	print(dmask)

	# Test HARDEST_BITMASK:
	hrep = PixelQualityFlags.binary_repr(PixelQualityFlags.HARDEST_BITMASK)
	assert hrep == '11111111111111111111111111111111'

	#for flag in PixelQualityFlags:
	#	print(flag)
	#	indx = PixelQualityFlags.filter(quality, flag)
	#	rep = CorrectorQualityFlags.binary_repr(quality)
	#	assert len(rep) == 32
